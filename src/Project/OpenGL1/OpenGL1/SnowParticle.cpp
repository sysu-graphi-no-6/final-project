...
namespace SnowParticle {
	...
	SnowParticle::SnowParticle()
	{
		...
		mUpdateShader = new Shader("SnowUpdate.vs", "SnowUpdate.fs",
			"SnowUpdate.gs", varyings, 5);
		//设置TransformFeedback缓存能够记录的顶点的数据类型

		mRenderShader = new Shader("SnowRender.vs", "SnowRender.fs");
		//设置随机纹理
		InitRandomTexture(512);
		mSparkTexture.loadTexture("雪花.png");
		mRenderShader->use();
